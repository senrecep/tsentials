...
describe('ResultChain sync API', () => {
  ...
  it('bind short-circuits on failure', () => {
    ...
    const r = chain(Result.failure<number>(err))
      .bind(n => { called.push(true); return Result.success(n * 2); })
      .unwrap();
  ...
  it('map passes through failure', () => {
    const called: boolean[] = [];
    const r = chain(Result.failure<number>(err))
      .map(n => { called.push(true); return n + 1; })
      .unwrap();
    expect(r.ok).toBe(false);
    expect(called).toHaveLength(0);
  });
  ...
  it('ensure supports function error', () => {
    const r = chain(Result.success(3))
      .ensure(n => n > 5, n => Err.validation('Value', `Bad ${n}`))
      .unwrap();
    expect(r.ok).toBe(false);
  ...
  it('match extracts error value', () => {
    const v = chain(Result.failure<number>(err)).match(() => -1, errs => errs[0]!.code);
  ...
  it('compensate passes through success', () => {
    const called: boolean[] = [];
    const r = chain(Result.success(5))
      .compensate(() => { called.push(true); return Result.success(0); })
      .unwrap();
    expect(r.ok).toBe(true);
  ...
  it('else calls factory on failure', () => {
    const r = chain(Result.failure<number>(err)).else(errs => errs.length).unwrap();
    expect(r.ok).toBe(true);
  ...
  it('mapError passes through success', () => {
    const called: boolean[] = [];
    const r = chain(Result.success(5))
      .mapError(errs => { called.push(true); return errs; })
      .unwrap();
    expect(r.ok).toBe(true);
    expect(called).toHaveLength(0);
  });
...
describe('ResultChain async API', () => {
  ...
  it('thenAsync short-circuits on failure', async () => {
    ...
    const c = await chain(Result.failure<number>(err))
      .thenAsync(async n => { called.push(true); return Result.success(n * 10); });
  ...
  it('mapAsync passes through failure', async () => {
    const called: boolean[] = [];
    const c = await chain(Result.failure<number>(err)).mapAsync(async n => { called.push(true); return n * n; });
    expect(c.unwrap().ok).toBe(false);
    expect(called).toHaveLength(0);
  });
  ...
  it('compensateAsync passes through success', async () => {
    const called: boolean[] = [];
    const c = await chain(Result.success(5))
      .compensateAsync(async () => { called.push(true); return Result.success(77); });
    expect(c.unwrap().ok).toBe(true);
